...
| 2026-08-08 | [Does `--compose` make a run faster?](./2026-08-08-compose-before-after.md) | No. Median wall 21.8m to 25.2m across six blind runs, ranges overlapping, because the clock is deliberation and the table was never the expensive part. Kept as a correctness feature: the tally can no longer disagree with the table it counts. |
| 2026-09-02 | [Does claims-only mode make a scored run cheaper?](./2026-09-02-claims-only-delta.md) | On the clock, no: median 22.2m full against 22.0m claims-only, six blind runs, overlapping ranges, quality in the same band. Kept as the eval arm because the claims file is the only artifact the scorer reads, and it gets the same quote gate. Every run failed the known-true gate, published as read. |
